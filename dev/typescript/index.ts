...
//WinJS type override
interface WinJSHandledEvent extends WinJS.Application.IPromiseEvent {
	handled?: boolean
}
  
//Nyckel template types

type vue_detailsView = {
	id: string,
	table: string,
	image: string,
	data: vue_detailsViewItem,
	title: string,
	subtitle: string,
	error?: string
};
type vue_listViewItem = {
	id: string,
	table: string,
	sortBy: string,
	text: string,
...
type template_title = string | string[] | { text: string | string[], joiner?: string };

type template_dropdownOptions = {
	listInput: string[],
	textInput: boolean
};

type vue_dropdownListItem = {
	action: string,
	text: string,
	icon?: string,
	disabled?: boolean,
...
interface template_detailsViewItem {
	column: string,
	label?: {
		column: string,
		dropdownList?: template_dropdownOptions
	},
	hidden?: boolean,
	readonly?: boolean,
	dropdownList?: template_dropdownOptions,
...
interface vue_detailsViewItem extends template_detailsViewItem {
	column: string,
	orig: tableValue,
	text: tableValue[],
	type: typeString,
	splitter?: string,
	options?: vue_dropdownListItem[],
	textInput?: boolean,
	label?: {
		orig: tableValue,
		text: tableValue,
...
interface template_detailsViewGroup {
	groupHeading: string,
	group: (string | {
		column: string,
		label?: {
...
interface vue_detailsViewGroup extends template_detailsViewGroup {
	groupHeading: string,
	group: {
		column: string,
		text: tableValue[],
...
interface nyckelDBInitTemplate {
	headers?: initTableHeaders,
	types?: initTableProperties,
	options?: {
	...
	display?: {
		...
		heading?: {
			...
			subtitle?: template_title
		},
		detailsView?: (string | template_detailsViewItem | template_detailsViewGroup)[]
	}
}
...
interface vue_state {
	...
	//navigation
	views: { [pageName: string]: vue_navPage },
	backArrow: boolean,
	currentView: vue_navPage,
	...
	//page animations
	transitionName: string,// "forward" | "back",
	...
	loggedIn: boolean,
	stoKey: string,
	showStoKeyInput: boolean,
	stoKeyWarning: string,
	...
	searchResults: vue_listViewItem[],
	searchResultsTitle: string,
	searchResultsError: string,
	currentQuery: string,

	//details view
	details: vue_detailsView,
	recentlyViewed: vue_listViewItem[],

	//groups
	addItemToGroupDropdown: boolean,
	groupDropdown: boolean,
	groupSearchBox: string,
	groups: string[],
	showNewGroupUI: boolean,
	activeGroup: vue_listViewItem[]
}

interface HTMLHiddenDownloadLinkElement extends HTMLLinkElement {
	download: string
}

interface CordovaAppNavigator extends Navigator {
	app: {
		exitApp: Function
	}
}

interface experimentalStyleSheet extends StyleSheet {
...
type wwCommand = keyof nyckelFunctions | "initNewNyckelDB" | "read" | "write" | "readShared" | "share" | "exists" | "stop";
type wwManagerInput = { cmd: wwCommand, args?: any[], title?: string };
...
var APP, VAL: { toPropName: Function, toEnglishAlphabet: Function, removeHTMLTags: Function }, appData: {[NyckelDBTitle: string]: API} = {}, Vue: VueConstructor, getWidth: Function, getHeight: Function, csv2json: Function, Windows, cordova; //dependancies
(function () {
	"use strict";
	...
	const DROPBOX_CLIENT_ID = "jk6tb5tp76hs2tx",//get new client id from https://www.dropbox.com/developers
		...
		dataTemplates: {[templateName: string]: nyckelDBInitTemplate } = {//add, remove or edit NyckelDB dataTemplates to suit your app needs. Examples below for contacts, files and groups NyckelDB databases
			Contacts: {
				...
				display: {
					...
					detailsView: [//displays the following content in the following order
						...
						{ column: "GroupMembership", hidden: true, readonly: true },
						{
							...
							group: [
								"Birthday",
								{ column: "Event1_Value", label: { column: "Event1_Type" } },//here, the value of Event1_Value is the text column and the value of Event1_Type is 
								{ column: "Event2_Value", label: { column: "Event2_Type" } },//the label column when column and label are specified 
			...
			Files: {//these headers are given as an array
				...
				types: {//when headers are an array, types can be given as a corresponding array, or object
					"Display Name": "string",
					"Name": "string",
					"Extension": "string",
					"Type": "string",
		...
		freshStateObj = function (): vue_state {
	...
	var fileReaderInitiated: boolean[] = [],
		backstack: string[] = [],
		backIndex = 0,
		state: vue_state = freshStateObj(),
		...
		setNavLinkIndicatorPosition = function (location: string) {
			...
			if (app.views[location] && app.views[location].level === 1) {//show indicator and move to correct position
				var sidenavlink = (document.getElementById("sidenavlink_" + location) as HTMLElement).getBoundingClientRect(),
					topnavlink = (document.getElementById("topnavlink_" + location) as HTMLElement).getBoundingClientRect(),
					views = (document.getElementById("topnav-container") as HTMLElement).getBoundingClientRect(),
		 ...
		 * }
		 */
		wwManager = function (inputObj: wwManagerInput, callback?: Function, finalCallback?: Function) {
			...
			function noWebWorker() {
				function applyCallback(callback: Function) {
				...
				if (obj.title && obj.cmd) {
					...
					if (obj.cmd === "initNewNyckelDB") {
						...
						else debug("NyckelDB not initialised with correct arguments");
					}
			...
			var obj: {
				...
				finalCallbackIndex?: number
			} = inputObj;
			if (!!window.Worker && !localTestingMode) startWorker();
			else noWebWorker();
		},
		wwReadMessage = function (e: MessageEvent) {
		...
		wwOnError = function (e: ErrorEvent) {
			debug(e.message, "Web Worker error: " + e.filename + ': ' + e.lineno);
		},
		defaultErrorHandler = function (success:boolean, errors: string) {
			if (errors) {
		...
		startApp = function (resumeBool?:boolean) {
			function doneLoadingApp() {
				setTimeout(function () {
					setNavLinkIndicatorPosition(app.currentView.path);
					(document.getElementById("loading") as HTMLElement).className = "done"; //app is rendered so fade in from black
				}, 150);
				checkDBLoaded(function (nextInQueue: Function) {
			...
			function setupUI(callback: Function) {
			...
			function tryDropbox(cachedStoKey: string) {
				function applyUser(user: {alias: string, email: string, dbid: string}) {
					if (user) {
			...
			function getLocalState() {
				APP.Sto.getItem("state", null, function (appStateObj: appState, error:string) {
					if (appStateObj) {
		...
		*/
		colorLuminance = function (hex: string, lum: number, returnRGB: boolean): string {
			// validate hex string
			...
			var hexColor = "#", rgbColor: number[] = [], c, i, black = 0, white = 255;
			for (i = 0; i < 3; i++) {
		...
		updateCSSColor = function (rgbColor: string, replaceColor: RegExp) {
			var styleSheets = document.styleSheets;
			try {
				for (var a = 0, lenA = styleSheets.length, b, lenB, c, lenC, rules, rule, styles; a < lenA; a++) {
					rules = (styleSheets[a] as experimentalStyleSheet).rules || (styleSheets[a] as experimentalStyleSheet).cssRules;
		...
		setAccentColor = function (hex): boolean {
			if (!hex) return false;
			var colors = [
				...
				success = false;
			if (metaThemeColor) metaThemeColor.setAttribute("content", hex);
			if (appleThemeColor) appleThemeColor.setAttribute("content", hex);
			if (windowsThemeColor) windowsThemeColor.setAttribute("content", hex);
		...
		windowsAccentColor:string[] | false[] = [false, false, false, false, false, false, false],
		matchSystemTheme = function () {
			...
			function isFontAvailable(font: string) {
				...
				var getWidth = function (fontFamily: string) {
			...
			if (isFontAvailable('Segoe UI Symbol')) {
				(document.getElementById("segoe-icons") as HTMLLinkElement).rel = 'stylesheet';
				(document.getElementById("default-icons") as HTMLLinkElement).rel = 'alternate stylesheet';
			}
			if (Windows) {
		...
		loadDB = true,
		loadDBQueue: Function[] = [],
		loadingDB = false,
		checkDBLoaded = function (callback: Function) {
			function initDB(title: string, template: nyckelDBInitTemplate, dbNum: number, numOfTables: number) {
				var options: {
					customProperties?: customProperties,
					doNotIndex?: string[],
					initialIndex?: string[],
				...
				var cb = function (success: boolean, errors: string) {//default callback function for handling errors initialising NyckelDB
					if (errors) defaultErrorHandler(success, errors);
				};
				if (numOfTables === dbNum + 1) {
					cb = function (success: boolean, errors: string) {//final callback function for last NyckelDB to initialise
						if (errors) defaultErrorHandler(success, errors);
						...
						else if (nextInQueue instanceof Function) nextInQueue(cb);
					};
				}
		...
		createNewItem = function (tableName: string) {
			//get blank details object by just provinding a table and no id
			getDetails({ table: tableName }, function (detailsObj: vue_detailsView) {
				app.details = detailsObj;
				app.navigate("edit");
			});
		},
		formatValue = function (text: tableValue, type: typeString, splitter: string): tableValue[] {
			var ret: tableValue[] = []
			...
			if (typeof text === "string") {
				...
				else ret = splitter ? text.split(splitter) : text.split(" ::: ");
			}
			else if (text !== undefined) ret = [text];
			return ret;
		},
		getDetails = function (returnedJSON, callback) {
			function processDetailsReturnData(row, error, cb) {
				function getDetailsData(template, splitter, labelDropdownList?) {
					function getDropdownList(optionsObj): vue_dropdownListItem[] {
						var dropdownList: vue_dropdownListItem[] = [];
					...
					function applyValueArr() {
						...
						for (let a = 0, lenA = template.column.length; a < lenA; a++) {
							if (row[template.column[a]].value) vueDetailsData.text[a] = row[template.column[a]].value;
						}
					...
					function applyValueObj() {
						...
						if (template.dropdownList) {
							if (template.dropdownList.listInput) vueDetailsData.options = getDropdownList(template.dropdownList);
							if (template.dropdownList.textInput) vueDetailsData.textInput = true;
						}
					}
					function applyLabel() {
						...
						if (labelDropdownList) {
							if (labelDropdownList.listInput) vueDetailsData.label.options = getDropdownList(labelDropdownList);
							if (labelDropdownList.textInput) vueDetailsData.label.textInput = true;
						}
					}
					var vueDetailsData: vue_detailsViewItem = {
						column: "",
						orig: "",
						text: [],
						type: "any"
					...
					if (typeof template === "string") applyValueStr();
						...
						if (template.column.constructor === Array) applyValueArr();
						else applyValueObj();
						if (template.label) applyLabel();
						if (template.readonly) vueDetailsData.readonly = true;
						if (template.hidden) vueDetailsData.hidden = true;
						vueDetailsData.splitter = splitter;
					}
					return vueDetailsData;
				}
				function getGroup(template) {
					var ret: vue_detailsViewGroup = {
						groupHeading: "",
						group: [],
					...
					for (let c = 0, lenC = template.group.length, d, lenD; c < lenC; c++) {
						...
						for (d = 0, lenD = ret.group[c].text.length; d < lenD; d++) {
							if (ret.group[c].text[d] !== "") ret.show = c + 1;
						}
					}
				...
				function getHeading(template) {
					...
					if (typeof template === "string") ret = row[VAL.toPropName(template)].value;
						...
						for (let a = 0, lenA = template.text.length; a < lenA; a++) {
							if (row[template.text[a]].value) ret[a] = row[template.text[a]].value;
						}
						ret = ret.join(template.joiner || " ");
					}
					else debug("template heading error. change heading 'value' to 'text'");
					return ret;
				}
				var data = [],
					b = 0,
			...
			function afterDBLoaded(nextInQueue) {
				app.spin(true, "Loading data...");
				app.storeState();
				var wwInput: wwManagerInput = {
					cmd: returnedJSON.id ? "getRow" : "getRowTemplate",
					title: returnedJSON.table,
					args: []
				};
				if (returnedJSON.id) wwInput.args = [returnedJSON.id];
				wwManager(wwInput, function (row, error) { processDetailsReturnData(row, error, nextInQueue); });
			}
			checkDBLoaded(afterDBLoaded);
		},
		generateListItems = function (tableName, ids, options?: { sortByCol?: string, selected?: boolean }, callback?: (list:vue_listViewItem[]) => void) {
			function buildList(result, errors) {
				var ret: vue_listViewItem[] = [];
			...
			options = options || {};
			var columns = dataTemplates[tableName].display.listView.text.join("|||").split("|||"),
				joiner = dataTemplates[tableName].display.listView.joiner ? dataTemplates[tableName].display.listView.joiner : " ";
		...
		generateList = function (dbTitle, ids, options, callback?: (list:vue_listViewItem[]) => void) {
		...
		buildMailtoUri = function (to?, bcc?, subject?, message?, callback?) {
			...
			if (callback instanceof Function) return callback("mailto:" + to + query + bcc + joiner1 + subject + joiner2 + message, bccBool);
		...
		getHeadersArr = function (headers: initTableHeaders): string[] {
				...
				updateDropdownLinks: function () {
					...
					function getEmails(ids) {
						...
						if (ids.length > 0) wwManager({
							...
							var type, email, name, primary: boolean | number = false;
							...
							if (emailAddresses.length > 0) {
								if (emailAddresses.length === 1) buildMailtoUri(emailAddresses[0], null, null, null, function (uri) { updateLinks.call(this, uri, ids); }.bind(this));
								else buildMailtoUri(app.dropboxEmail || "", emailAddresses.join(","), null, null, function (uri) { updateLinks.call(this, uri, ids); }.bind(this));
							}
					...
					function matches(subsetArr, ofArr) {
						...
						if (ofArr && ofArr.constructor === Array) {
							for (let a = 0, len = subsetArr.length; a < len; a++) {
								if (ofArr.indexOf(subsetArr[a]) === -1) ret = false;
					...
					function saveFile(str, fileName, mimeType) {
						...
						function downloadToBrowser(str, fileName, mimeType) {
							...
							if (/Macintosh|iPad|iPod|iPhone/.test(navigator.userAgent)) {
								...
								var link = (document.getElementById("hiddenDownloadLink") as HTMLHiddenDownloadLinkElement);
								if (link) {
				...
				saveChanges: function () {
					...
					function save(data, label) {
						if (data.text) {
							...
							else if (typeof data.text === "string") text = data.splitter ? data.text.join(data.splitter) : data.text.join(" ::: ");
							else text = data.text[0];
							if (text !== data.orig) {
								//set value 
	...
	const app = new Vue({
		...
		methods: {
			...
			navigate: function (location?, searchquery?, detailsObj?) {
				var query: { page: number, search?: string, id?: string, table?: string } = { page: backIndex + 1 };
				if (searchquery) {
				...
				else if (location && "#/" + location + "?page=" + backIndex === window.location.hash) return; //dont navigate if no change				
			...
			search: function (event, optionalQuery) {
				...
				if (!searchBoxInputElement) return;
				if (!this.showSearchBar) {
					...
					if (this.searchBox === "debugmode") {
						...
						checkDBLoaded(function (nextInQueue) {
							...
							var find = optionalQuery ? optionalQuery : this.searchAutoComplete === "" ? this.searchBox : this.searchAutoComplete,
								results = [], appElement = document.getElementById("app");
							find = trim(find);
							if (find) {
			...
			applySuggestion: function (str) {
				...
				this.searchPointer = -1;
				var searchBoxInputElement = document.getElementById("searchBox");
				if (searchBoxInputElement) searchBoxInputElement.focus();
			},
			...
			newStoKey: function () {
				var key = (document.getElementById("stoKeyInput") as HTMLInputElement),
					confirmKey = (document.getElementById("stoKeyInputConfirm") as HTMLInputElement);
				checkDBLoaded(function (nextInQueue) {
					if (key.value === "") {
			...
			updateStoKey: function () {
				...
				confirm("Please input your current password", function () {
					checkDBLoaded(function (nextInQueue) {
						var key = (document.getElementById("updateStoKeyInput") as HTMLInputElement);
						if (dbid) {
							this.stoKey = Base64.hash(dbid + key.value);
						}
			...
			loadFile: function (fileInputId, fileExtension, callback) {
				...
				function initFileReader(readAs, callback) {
					function init() {
						...
						var file = fileInputElement.files[0],/* FileList object*/
							path = fileInputElement.value.replace(/\\/g, "/"),
							ext,
				...
				function saveFile(source, details) {
					var contents = Base64.write_and_verify(source, Base64.hash(this.stoKey)),
						...
						ext = /(?:\.([^.]+))?$/.exec(details.name),
						extension = ext ? ext[1] : "No file extension",
						...
						if (source.Rows[0].Notes) {
							...
							for (var d = 0, lenD = notes.length; d < lenD; d++) {
								if (/^\-Imported/.test(notes[d])) {
									var impFr = notes[d].replace(/^\-Imported/, ""),
										importDate:number[] = [];
				...
				var fileInputElement = (document.getElementById(fileInputId) as HTMLInputElement);
				checkDBLoaded(function (nextInQueue) {
	...
	if (Windows && WinJS) {
		...
		winApp.onactivated = function (args) {
			...
			WinJS.Application.onbackclick = function (e:WinJSHandledEvent) {
				e.handled = true;
				return APP.goBack();
			};
		...
		var onDeviceReady = function () {
			...
			function onBack() {
				if (!APP.goBack() && navigator && (navigator as CordovaAppNavigator).app && (navigator as CordovaAppNavigator).app.exitApp) (navigator as CordovaAppNavigator).app.exitApp();
			}
