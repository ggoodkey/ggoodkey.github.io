...
(function (root, factory) {
	...
	var dropbox = function (endpoint, apiArgs) {
		var args = [].slice.call(arguments),
			config = endpointMapping[endpoint] || {},
		...
		if (baseUri === 'sharing/get_shared_link_file') r.responseType = '';
		...
		if (apiArgs && (format === 'content-upload' || format === 'content-download')) r.setRequestHeader('Dropbox-API-Arg', JSON.stringify(apiArgs));
	...
	dropbox.setTokenStore = function (store) { tokenStore = store; },
		dropbox.authenticate = function (apiArgs, handlers) {
			...
			var params = paramsFromUrlHash(window.location.hash),
			...
			if (params.state && csrfToken && params.state === csrfToken) {
				...
				if (cordova) { /*! requires Cordova inAppBrowser plugin */
