//depenancies
var APP, VAL, Base64, Windows, cordova, Spelling;
var NyckelDB = /** @class */ (function () {
    /**
     ...
     */
    function NyckelDB(tableTitle, tableHeaders, columnProperties, options, callback) {
        this.version = "0.4";
        /**
         ...
         */
        this.search = function (searchQuery, options, callback) {
            function findMatches(arr, min) {
                min = min || 2;
                if (min === 1)
                    return arr;
                var ret = [], /*return array*/ a = 0, found = false;
                arr = arr.sort();
                for (var x = 0, y = x + 1, len = arr.length, b = void 0; x < len - min + 1; x++, y = x + 1) {
            ...
            function querySearchIndex(searchQuery, lastRoundBool, callback) {
                ...
                for (var a = 0, b = void 0, bLen = NyckelDB.COL_NAMES_INDEXED[this.id].length, i = void 0; a < aLen; a++) {
                    for (b = 0, i = []; b < bLen; b++) {
                ...
                if (lastRoundBool)
                    return callback instanceof Function ? callback(ids, NyckelDB.ERRORS[this.id], NyckelDB.DB[this.id].title, this.syncPending) : ids;
            }
            function fuzzyMatch(searchQuery, callback) {
                //collect all possible alternate queries
                for (var a_1 = 0, lenA = searchQuery.length; a_1 < lenA; a_1++) {
                    if (Spelling[searchQuery[a_1]]) {
                        searchQuery[a_1] = Spelling[searchQuery[a_1]].split(" ");
                    }
                    else
                        searchQuery[a_1] = [searchQuery[a_1]];
                }
                //combine them in all possible combinations
                var queries = [], a = 0, n = 0, cursor = [], len = searchQuery.length;
                while (len)
                ...
                for (var a_2 = 0, len_1 = queries.length - 1; a_2 < len_1; a_2++) {
                    querySearchIndex.call(this, queries[a_2]);
                }
                return querySearchIndex.call(this, queries[queries.length - 1], true, callback);
            }
            function search(searchQuery, callback) {
                if (options.fuzzyMatch)
                    fuzzyMatch.call(this, searchQuery, callback);
                else
                    querySearchIndex.call(this, searchQuery, true, callback);
            }
            if (this.isDeleted())
                return callback instanceof Function ? callback([], "no data", NyckelDB.DB[this.id].title, this.requiresSync) : [];
            options = options || {};
            var cols = options.colNames ? options.colNames.join("|").split("|") : null, ids = [];
         ...
         * @returns {array} an Array of row ids which contain the searchQuery
         */
        this.advancedSearch = function (searchQuery, options, callback) {
            ...
            if (!searchQuery)
                return callback instanceof Function ? callback([], "no query supplied", NyckelDB.DB[this.id].title, this.requiresSync) : [];
            var ids = [], filterIds = [], filterOutQueries = [];
            if (/\s\+|\s\-/.test(searchQuery)) {
                searchQuery = searchQuery.split(" +");
                for (var a = 0; a < searchQuery.length; a++) {
         ...
         */
        this.getSearchSuggestions = function (searchQuery, options, callback) {
            function buildSuggestionsList() {
                ...
                if (last) {
                    ...
                    for (var b = 0, len = NyckelDB.SEARCH_SUGGESTIONS[this.id].length; b < len; b++) {
                    ...
                    matchesThis = null;
                    for (var c = 0, lenC = NyckelDB.RECENTLY_SEARCHED[this.id].length, i = void 0, item = void 0; c < lenC; c++) {
            ...
            if (NyckelDB.SEARCH_SUGGESTIONS[this.id].length > 0) {
                ...
                if (callback instanceof Function)
                    callback([], "currently busy building search index", NyckelDB.DB[this.id].title, this.requiresSync);
         ...
         * @returns {object} this
         */
        this.forEachRow = function (funct, callback) {
         ...
         * @returns {object} this
         */
        this.forEachCol = function (funct, callback) {
            if (funct instanceof Function) {
                for (var a = 1, headers = NyckelDB.DB[this.id].columns.$headers, len = headers.length; a < len; a++) {
         ...
         */
        this.getTitle = function () { return NyckelDB.DB[this.id] ? NyckelDB.DB[this.id].title : undefined; };
        /**
        ...
        this.setTitle = function (newTitle, callback) {
            var oldId = this.id, oldTitle = NyckelDB.DB[this.id].title, y = NyckelDB.DBS.indexOf(oldTitle);
            ...
            return this.INITIATE_NEW_DB.call(this, newTitle, function (newTitle) {
                ...
                if (NyckelDB.DB[this.id].previousTitle)
                    NyckelDB.DB[this.id].previousTitle.push(VAL.toPropName(oldTitle));
                else
                    NyckelDB.DB[this.id].previousTitle = [VAL.toPropName(oldTitle)];
                //delete old table
         ...
         * @returns {object} this
         */
        this.sortByCol = function (colName, options) {
         ...
         */
        this.getVal = function (rowId, colName) {
            var rowIndex = this.GET_INDEX_OF_ROW.call(this, rowId), colIndex = this.GET_INDEX_OF_COLUMN.call(this, colName);
            if (rowIndex > -1 && colIndex > 0)
                return NyckelDB.DB[this.id].table[rowIndex][colIndex];
        };
        /**
         ...
         */
        this.getVals = function (rowIds, colNames, callback) {
         ...
         */
        this.getRow = function (rowId, callback) {
            var rowIndex = this.GET_INDEX_OF_ROW.call(this, rowId), ret = {};
            if (rowIndex > -1) {
                this.forEachCol(function (colName, is) {
                    ...
                    if (NyckelDB.DB[this.id].columns[colName].exportAs)
                        ret[colName].column = NyckelDB.DB[this.id].columns[colName].exportAs[0];
                ...
                return callback instanceof Function ? callback(false, "row id not found", NyckelDB.DB[this.id].title, this.syncPending) : false;
        };
        /**
        ...
        * @param {getRowCallback} [callback] callback
        */
        this.getRowTemplate = function (callback) {
            var ret = {};
        ...
        */
         ...
         */
        this.toJSON_KeyValuePairs = function () {
            //TODO
            return "function not complete";
        };
        /**
        ...
        this.shuffle = function () {
            ...
            if (!this.isDeleted()) {
                var len = this.getLength(), shuffled = shuffle(NyckelDB.DB[this.id].table);
         ...
         */
        this.getIndexOf = function (id, orValue, colName) {
            if (this.isDeleted())
                return -1;
            else if (id) {
         ...
         * @returns {object} this
         */
        this.filter = function (colName, regExp) {
            if (this.getLength() > 0) {
                var colIndex = this.GET_INDEX_OF_COLUMN.call(this, colName), val;
                if (colIndex > 0) {
                    for (var a = 0, arrLen = NyckelDB.DB[this.id].table.length; a < arrLen; a++) {
        ...
        this.addRow = function (array, id) {
            if (array && array.constructor !== Array && typeof array === "object") {
                ...
                return this.ADD_ROW.call(this, array, id);
        };
        /**
         ...
         */
        this.setVal = function (rowId, colName, newValue, callback) {
        ...
        * @param {setCallback} [callback] callback function
        */
        this.setVals = function (rowId, newValues, callback) {
            function ret() {
                a++;
                if (a === len && callback instanceof Function)
                    return callback(true, NyckelDB.ERRORS[this.id], NyckelDB.DB[this.id].title, true);
            }
            var len = 0, a = 0;
         ...
         * @returns {object} this
         */
        this.importJSON = function (json, syncKey, syncToken, callback) {
            return this.IMPORT_JSON.call(this, json, function (success, errors, title, syncChanges) {
                if (success && syncChanges && !errors)
                    return this.CREATE_BASE64_FILE.call(this, syncKey, syncToken, callback), this;
                else
         ...
         * @returns {object} this
         */
        this.deleteTable = function (callback) {
         ...
         * @param {successCallback} [callback] callback function
         */
        this.NUKEALL = function (msg, callback) {
            function nuke() {
                if (APP.Sto) {
                    for (var a = 0, len = NyckelDB.DBS.length; a < len; a++) {
         ...
         */
        this.isSyncPending = function (cloudSyncFile, callback) {
            function ret(val) {
         ...
         * @return {object} this
         */
        this.sync = function (json, options, callback) {
            ...
            function sync() {
                ...
                return this.IMPORT_JSON.call(this, json, function (success, errors, title, changes) {
                    if (success && !errors && (changes || this.syncPending === true || forceSync === true)) {
                        ...
                        return callback instanceof Function ? (callback(true, NyckelDB.ERRORS[this.id], title, false), this) : this;
                }.bind(this));
            }
            ...
            options = options || {};
            var forceSync = options.forceSync || false, readKey = options.key || false, writeKey = options.key || false, wait = NyckelDB.MAX_SYNC_FREQUENCY + NyckelDB.DBX_SYNC_OBJ[NyckelDB.DB[this.id].title] - this.TIMESTAMP();
            if (options === true)
                forceSync = true;
            keyMigration.call(this);
            if (wait > 0 && !forceSync)
                return retError.call(this, "rate limited, try again in " + wait + " minutes");
            else if (NyckelDB.SYNC_ERROR && new Date().getTime() - NyckelDB.SYNC_ERROR_TIME < 6e4)
         ...
         */
        this.setType = function (colName, type, data, callback) {
         ...
         * @returns {boolean} whether or not sync is needed
         */
        this.setSyncCompleted = function (cloudSyncFile, callback) {
         ...
         * @since 0.4
         */
        this.addColumn = function (colName, position, options, callback) {
         ...
         * @since 0.4
         */
        this.deleteColumn = function (colName, callback) {
         ...
         * @since 0.4
         */
        this.renameColumn = function (colName, newName, callback) {
         ...
         * @since 0.4
         */
        this.validate = function (value, valueType, callback) {
            ...
            function validateEmail(email) {
                var orig = email, e = this.TRIM(String(email)).split("@");
                if (e.length === 2) {
                    //username
            ...
            if (!this.VALIDATE_TYPE.call(this, valueType))
                return ret.call(this, false, value, valueType + " is not a valid type");
            if (!this.VALUE_IS_VALID.call(this, value, valueType))
                return ret.call(this, false, value, NyckelDB.ERRORS.pop());
            if (value === "")
            ...
            for (var prop in props) {
                if (props.hasOwnProperty(prop)) {
                    ...
                    if (typeof props[prop] === "string") {
                        ...
                        if (this.VALUE_IS_VALID.call(this, props[prop][0], _type))
                        ...
                        this.CACHE_ERROR.call(this, prop, "invalid customProperty");
                }
            }
            _type = null;
            _initialValue = null;
        ...
        function applyData(json) {
            function setColumns(json) {
                if (json) {
                    ...
                    return this.APPLY_COLUMN_PROPERTIES.call(this, tableHeaders, columnProperties, this.TIMESTAMP(), options.doNotIndex);
            }
        ...
        function gotCachedTable(json) {
            function returnFunc(success, errors, title, syncChanges) {
                if (success && syncChanges && !errors)
                    ...
                    return callback(success, errors, NyckelDB.DB[this.id].title, false);
                else
                    return errors;
            }
            if (!json) {
        ...
        function didntGetCachedTable() {
            ...
            //validate ids
            for (var id in NyckelDB.DB[this.id].ids) {
                if (NyckelDB.DB[this.id].ids.hasOwnProperty(id)) {
                    for (var a = 0; a < NyckelDB.DB[this.id].ids[id].length; a++) {
            ...
            for (var prop in NyckelDB.DB[this.id].properties) {
            ...
            if (options.importJSON)
                this.IMPORT_JSON.call(this, options.importJSON, function (success, errors, title, syncChanges) {
                    return this.CREATE_BASE64_FILE.call(this, options.key, options.token, callback);
                });
            else {
                this.TO_LOCAL_STORAGE.call(this, true);
        ...
        if (tableTitle == null) { // eslint-disable-line
            ...
            if (options.importData && options.importData.data) {
                if (options.importData.version !== this.version + "_" + Base64.Version && options.importData.version !== this.version + "." + Base64.Version) {
                    ...
                    return callback instanceof Function ? callback(false, "imported databse corrupted", null, false) : "imported database corrupted";
            }
            else
                this.INITIATE_NEW_DB.call(this, tableTitle, initiateTable.bind(this));
        }
    }
    NyckelDB.prototype.IS_NUMERIC = function (n) {
    ...
    NyckelDB.prototype.DELETE_DUPLICATES = function (arr) {
    ...
    NyckelDB.prototype.TRIM = function (str) {
    ...
    b/t wRefTo and Now. */
    NyckelDB.prototype.TIMESTAMP = function (wRefTo) {
    ...
    NyckelDB.prototype.READABLE_TIMESTAMP = function () {
    ...
    NyckelDB.prototype.READABLE_JSON_STRING = function (str) {
        function enter() {
            out += "\r\n";
    ...
    NyckelDB.prototype.ROW_IDS_IS_VALID = function (rowId) {
        if (this.isDeleted())
            return false;
        if (this.IS_NUMERIC(rowId)) {
            ...
            else if (rowId < NyckelDB.DB[this.id].table.length)
                return true;
            else
                return false;
        }
        else if (typeof rowId === "string" && /^[a-z0-9]+$/i.test(rowId)) {
            if (NyckelDB.DB[this.id].ids[rowId])
                return true;
            else
                return false;
        }
    ...
    NyckelDB.prototype.COL_NAME_IS_VALID = function (colName) {
        ...
        else if (this.IS_NUMERIC(colName))
    ...
    NyckelDB.prototype.VALUE_IS_VALID = function (value, type, ignoreErrors) {
        if (value == null) { // eslint-disable-line eqeqeq
            if (!ignoreErrors)
                this.CACHE_ERROR.call(this, value, type + " value cannot be");
            return false;
        }
        else if (type === "any")
            return true;
        else if (typeof value === "string" && type.match(NyckelDB.VALID_STRING_TYPES))
            return true;
        else if (this.IS_NUMERIC(value) && type.match(NyckelDB.VALID_NUMBER_TYPES))
            return true;
            ...
            if (!ignoreErrors) {
                if (typeof type !== "string")
                    this.CACHE_ERROR.call(this, type, "Type must be supplied as a string");
                else if (!type.match(NyckelDB.VALID_TYPES))
                    this.CACHE_ERROR.call(this, type, "Invalid data type");
                else
                    this.CACHE_ERROR.call(this, value + " is a " + typeof value + ", not a " + type, "invalid value");
            }
            return false;
        }
    };
    NyckelDB.prototype.GET_INDEX_OF_ROW = function (rowId) {
        function getIndex(rowId) {
            for (var a = 0, len = NyckelDB.DB[this.id].table.length; a < len; a++) {
                NyckelDB.ROW_INDEX_CACHE[this.id][NyckelDB.DB[this.id].table[a][0]] = a; //build a cache for faster performance
                if (NyckelDB.DB[this.id].table[a][0] === rowId)
                    return a;
            }
        }
        if (this.isDeleted())
        ...
        if (this.ROW_IDS_IS_VALID.call(this, rowId)) {
            if (this.IS_NUMERIC(rowId))
                ...
                if (NyckelDB.DB[this.id].ids[rowId][0] === "del")
                    return -1;
                if (NyckelDB.ROW_INDEX_CACHE[this.id] && NyckelDB.ROW_INDEX_CACHE[this.id][rowId])
                    return NyckelDB.ROW_INDEX_CACHE[this.id][rowId]; //direct from cache
                else
    ...
    NyckelDB.prototype.GET_INDEX_OF_HIDDEN_ROW = function (rowId) {
        if (NyckelDB.DB[this.id].hidden && NyckelDB.DB[this.id].hidenIds[rowId]) {
            if (NyckelDB.DB[this.id].hiddenIds[rowId][0] === "del")
                return -1;
            for (var a = 0, len = NyckelDB.DB[this.id].hidden.length; a < len; a++) {
                if (NyckelDB.DB[this.id].hidden[a][0] === rowId)
                    return a;
            }
        }
        else
    ...
    NyckelDB.prototype.GET_INDEX_OF_COLUMN = function (colName) {
        if (this.isDeleted())
            return -1;
        var headers = NyckelDB.DB[this.id].columns.$headers;
        if (this.IS_NUMERIC(colName) && colName > 0 && colName < headers.length)
            return colName;
        var orig = colName;
        colName = VAL.toPropName(colName);
        if (colName === "id")
    ...
    NyckelDB.prototype.EXPORT_DB = function () {
        if (!NyckelDB.DB[this.id].hidden)
            return NyckelDB.DB[this.id];
        else {
            var db = JSON.parse(JSON.stringify(NyckelDB.DB[this.id]));
            var row;
    ...
    NyckelDB.prototype.TO_LOCAL_STORAGE = function (changes) {
    ...
    */
    NyckelDB.prototype.IMPORT_JSON = function (json, callback, key, fromLocalStorageBool) {
        function ret(success, err, changes) {
            if (callback instanceof Function)
                return callback(success, err, NyckelDB.DB[this.id] && NyckelDB.DB[this.id].title, !err ? false : changes), this;
            else
                return this;
        }
        function applyJSON(json) {
            ...
            function syncColumns(columns, callback) {
                ...
                for (var a = 0, lenA = columns.$headers.length, colIndex = void 0, colName = void 0, prop = void 0; a < lenA; a++) {
                    ...
                    if (colIndex === -1)
                        this.ADD_COLUMN.call(this, colName, a, columns[colName], false, columns.$created[a]);
                    if (colIndex > 0) { //make changes					
                        if (colIndex !== a)
                            this.MOVE_COLUMN.call(this, colName, colIndex, false);
                            ...
                            if (columns[colName][prop][1] > NyckelDB.DB[this.id].columns[colName][prop][1]) {
                                switch (prop) {
                                    case "deleted": //shouldn't find an unsynced deleted column here!!!
                                        ...
                                        break;
                                    case "type":
                                        this.SET_TYPE.call(this, colName, columns[colName][prop][0], null, false, columns[colName][prop][1]);
                                        break;
                    ...
                    function deleteRows() {
                            ...
                            if (json.properties && NyckelDB.DB[this.id].properties) {
                                ...
                                for (var prop in json.properties) {
                                    ...
                                    if (NyckelDB.DB[this.id].properties[_prop]) {
                                        ...
                                        else if (NyckelDB.DB[this.id].properties[_prop][1] && NyckelDB.DB[this.id].properties[_prop][1] !== json.properties[prop][1])
                                            syncChanges = true;
                                    }
                                    else
                                        this.CACHE_ERROR.call(this, "customProperty " + _prop + " not initialized");
                                }
                                _prop = null;
                            }
                        ...
                        for (var rowId in json.ids) {
                            if (json.ids[rowId][0] === "del" && NyckelDB.DB[this.id].ids[rowId] && NyckelDB.DB[this.id].ids[rowId][0] !== "del") {
                                if (json.ids[rowId][1] !== NyckelDB.DB[this.id].ids[rowId][0])
                                    syncChanges = true;
            ...
            function directLoadDB() {
                ...
                if (NyckelDB.DB[this.id].lastModified >= json.lastModified)
                    return ret.call(this, true, null, false);
            ...
            var eTitle = VAL.toPropName(NyckelDB.DB[this.id].title), nTitle = VAL.toPropName(json.title);
            ...
            if ("deleted" in json) {
                ...
                return ret.call(this, false, "json is not valid");
        ...
        function applyCSV(json) {
            function getId(jsonRow, searchLevel, remainingIds, idColName) {
                ...
                function toArray(json) {
                    var arr = [], headerName;
                    for (var a = 0, headers_1 = NyckelDB.DB[this.id].columns.$headers, len = headers_1.length - 1; a < len; a++) {
                        headerName = headers_1[a + 1];
                        arr[a] = json[headerName];
                        //add empty values to table for boolean (false), number (0) or string ("") values
                        if (arr[a] === undefined) {
                            if (NyckelDB.DB[this.id].columns[headerName].type[0] === "boolean")
                                arr[a] = false;
                            else if (/^number|nteger$|itude$/.test(NyckelDB.DB[this.id].columns[headerName].type[0]))
                                arr[a] = 0;
                            else
                                arr[a] = "";
                        }
                ...
                function toEditTimesArr(json, traceStr) {
                    ...
                    for (var a = 1, len = NyckelDB.DB[this.id].columns.$headers.length; a < len; a++) {
                        ret[a] = 0;
                    }
                    return ret;
                }
            ...
            function finish() {
                if (json.Rows.length > 0)
                    return ret.call(this, false, "CSV import not completed");
            ...
            var headers = this.CHECK_HEADERS_ARRAY.call(this, json.Headers), foundMatch = false;
            if ("lastModified" in json) {
                if (json.lastModified instanceof Date)
                    json.lastModified = json.lastModified.getTime();
                if (this.IS_NUMERIC(json.lastModified))
                    json.lastModified = (json.lastModified - 15e11) / 6e4;
                //	else json.lastModified = null;
            }
            //check for matching headers
            for (var a = 0, lenA = headers.length, b = void 0, lenB = void 0, existingHeaders = NyckelDB.DB[this.id].columns.$headers; a < lenA; a++) {
                foundMatch = false;
                ...
                if (foundMatch === false) {
                    for (var col in NyckelDB.DB[this.id].columns) {
        ...
        if (!json)
            return ret.call(this, false, "no json");
        if (typeof json === "string")
            json = JSON.parse(json);
        ...
        if ("title" in json)
            ...
            return ret.call(this, false, "json incompatible");
    };
    NyckelDB.prototype.CACHE_ERROR = function (error, description) {
        ...
        if (console && console.log)
            NyckelDB.DB[this.id] ? console.log(NyckelDB.DB[this.id].title + ": " + error) : console.log(error);
    };
    NyckelDB.prototype.SAVE_FILE = function (str, fileName, mimeType) {
    ...
    NyckelDB.prototype.BUILD_SEARCH_INDEX = function (colNamesToIndex, callback) {
        ...
        function start() {
            ...
            for (var x = 0, y = 1, z = 0, reps = indexItem.length; x < reps; x = z > 0 ? x + z : x + 1, y = x + 1, z = 0) {
                var foundMatch = true, row, col, i = NyckelDB.SEARCH_INDEX[this.id][indexItem[x][0]] = {};
        ...
        if (NyckelDB.BUILDING_SEARCH_INDEX[this.id]) {
            if (callback instanceof Function)
                NyckelDB.BUILDING_SEARCH_INDEX_QUEUE[this.id].push(callback);
            return;
        }
        ...
        if (NyckelDB.DB[this.id].columns.$indexable !== undefined) {
            if (!colNamesToIndex)
                colNamesToIndex = NyckelDB.DB[this.id].columns.$indexable.join("|").split("|");
    ...
    NyckelDB.prototype.STO_SEARCH_INDEX = function () {
    ...
    NyckelDB.prototype.SET_PROP = function (propName, value, editTime, storeBool) {
        if (this.isDeleted())
            return;
        ...
        if (NyckelDB.DB[this.id].properties[propName] && this.VALUE_IS_VALID.call(this, value, NyckelDB.DB[this.id].properties[propName][2])) {
            if (NyckelDB.DB[this.id].properties[propName] !== undefined) {
                ...
                this.CACHE_ERROR.call(this, propName, "invalid property name");
        }
        else
            this.CACHE_ERROR.call(this, value, "cannot set " + propName);
    };
    NyckelDB.prototype.ADD_ROW = function (array, id, storeBool, editTimesArr) {
    ...
    NyckelDB.prototype.SET_VAL = function (rowId, colName, newValue, storeBool, editTime, callback) {
        function applyVal(toTable, toIds, rowIndex) {
            ...
            if (thisModified > NyckelDB.DB[this.id].lastModified)
                NyckelDB.DB[this.id].lastModified = thisModified;
            this.syncPending = true;
            thisModified = null;
            rowIndex = null;
            colIndex = null;
        ...
        if (typeof newValue === "string") {
            ...
            if (newValue !== "" && !isNaN(newValue * 1))
                newValue = newValue * 1; //convert numbers in String form to Number form
            if (newValue === "true")
                newValue = true;
            if (newValue === "false")
                newValue = false;
        }
        var rowIndex = this.GET_INDEX_OF_ROW.call(this, rowId), colIndex = this.GET_INDEX_OF_COLUMN.call(this, colName), rowIsHidden = false;
        if (rowIndex < 0) {
        ...
        if (validationObj.valid) {
            if (rowIsHidden)
                return applyVal.call(this, NyckelDB.DB[this.id].hidden, NyckelDB.DB[this.id].hiddenIds, rowIndex);
            else
                return applyVal.call(this, NyckelDB.DB[this.id].table, NyckelDB.DB[this.id].ids, rowIndex);
        }
        else
            return callback instanceof Function ? callback(false, NyckelDB.ERRORS[this.id], NyckelDB.DB[this.id].title, this.syncPending) : false;
    };
    NyckelDB.prototype.DELETE_ROW = function (rowId, storeBool, editTime) {
        if (this.isDeleted()) {
            this.CACHE_ERROR.call(this, rowId, "could not delete row, table has been deleted");
            return false;
        }
        //get row index
        var index = this.GET_INDEX_OF_ROW.call(this, rowId), rowIsHidden = false;
        //if not found
        if (index === -1) {
            //check hidden rows
        ...
        //if found, delete row
        if (rowIsHidden)
            NyckelDB.DB[this.id].hidden.splice(index, 1);
        else
            NyckelDB.DB[this.id].table.splice(index, 1);
        NyckelDB.ROW_INDEX_CACHE[this.id] = {};
        index = null;
        editTime = this.VALIDATE_EDIT_TIME.call(this, editTime, null, "row", null, "deleteRow");
        if (rowIsHidden)
            NyckelDB.DB[this.id].hiddenIds[rowId] = ["del", editTime];
        else
            NyckelDB.DB[this.id].ids[rowId] = ["del", editTime];
        NyckelDB.DB[this.id].lastModified = editTime + NyckelDB.DB[this.id].created > NyckelDB.DB[this.id].lastModified ? editTime + NyckelDB.DB[this.id].created : NyckelDB.DB[this.id].lastModified;
        this.syncPending = true;
        if (storeBool !== false)
            this.TO_LOCAL_STORAGE.call(this, true);
    ...
    NyckelDB.prototype.VALIDATE_EDIT_TIME = function (num, createdDiff, type, id, traceStr) {
    ...
    NyckelDB.prototype.DELETE_TABLE = function (callback, editTime, storeBool) {
        var msg = "Are you sure you want to delete " + NyckelDB.DB[this.id].title + " ?", del = function () {
            this.DELETE_TABLE_BY_ID.call(this, this.id, editTime);
            this.syncPending = true;
            if (storeBool !== false)
                this.TO_LOCAL_STORAGE.call(this, true);
            if (callback instanceof Function)
                return callback(true, NyckelDB.ERRORS[this.id], NyckelDB.DB[this.id].title, true), this;
            else
                return this;
        }.bind(this), cancel = function () {
            if (callback instanceof Function)
                return callback(false, NyckelDB.ERRORS[this.id], NyckelDB.DB[this.id].title, false), this;
            else
                return this;
        }.bind(this);
        if (APP.confirm)
    ...
    NyckelDB.prototype.DELETE_TABLE_BY_ID = function (id, editTime) {
    ...
    NyckelDB.prototype.INITIATE_NEW_DB = function (title, callback) {
        ...
        APP.Sto && APP.Sto.getItem("tables", null, function (tables) {
            if (tables) {
                ...
                this.CACHE_ERROR.call(this, "couldn't find tables");
        }.bind(this), newDBS.bind(this));
    };
    NyckelDB.prototype.CHECK_HEADERS_ARRAY = function (headers) {
        var header, ret = [];
        for (var a_3 = 0, b_1 = 1, len_2 = headers.length; a_3 < len_2; a_3++) {
            for (b_1 = a_3 + 1; b_1 < len_2; b_1++) {
                if (headers[a_3] === headers[b_1])
                    this.CACHE_ERROR.call(this, headers[a_3], "duplicate header values");
            }
            ret[a_3] = VAL.toPropName(headers[a_3]);
        }
    ...
    NyckelDB.prototype.CREATE_BASE64_FILE = function (key, token, callback) {
        function dataString() {
        ...
        function syncFile() {
            var title = VAL.toPropName(NyckelDB.DB[this.id].title);
            if (!NyckelDB.DBX_SYNC_OBJ[title] || NyckelDB.DB[this.id].lastModified > NyckelDB.DBX_SYNC_OBJ[title])
                NyckelDB.DBX_SYNC_OBJ[title] = NyckelDB.DB[this.id].lastModified;
            ...
            if (token) {
                ...
                if (token.version === this.version + "_" + Base64.Version) {
                    if (token.signature && token.token && token.signature === Base64.hmac(token.token, key)) {
                        ...
                        for (var i in token) {
                            if (!NyckelDB.DBX_SYNC_OBJ[i])
                                NyckelDB.DBX_SYNC_OBJ[i] = token[i];
                        }
                    }
                    else {
                        this.SYNC_ERROR = true;
                    ...
                    this.CACHE_ERROR.call(this, "token version not supported");
            }
            var title = VAL.toPropName(NyckelDB.DB[this.id].title);
            if (!NyckelDB.DBX_SYNC_OBJ[title] || NyckelDB.DB[this.id].lastModified > NyckelDB.DBX_SYNC_OBJ[title])
                NyckelDB.DBX_SYNC_OBJ[title] = NyckelDB.DB[this.id].lastModified;
        ...
        if (NyckelDB.SYNC_ERROR && new Date().getTime() - NyckelDB.SYNC_ERROR_TIME < 6e4)
            return callback instanceof Function ? (callback(false, "try again later", NyckelDB.DB[this.id].title, false), this) : this;
        var obj = {
    ...
    NyckelDB.prototype.VALIDATE_TYPE = function (type, options) {
        ...
        if (type && type.match(NyckelDB.VALID_TYPES))
            return type;
        else {
            if (options)
                this.CACHE_ERROR.call(this, type, "customProperties can only be set to string, number, boolean, or any, not");
            else
                this.CACHE_ERROR.call(this, type, "invalid Header type");
            return "any";
        }
    };
    NyckelDB.prototype.APPLY_COLUMN_PROPERTIES = function (tableHeaders, columnProperties, tableCreated, doNotIndex) {
        ...
        var time = this.TIMESTAMP(tableCreated), obj = {
    ...
    NyckelDB.prototype.DELETE_COLUMN = function (colName, storeBool, editTime, callback) {
        var colIndex = this.GET_INDEX_OF_COLUMN.call(this, colName);
        if (colIndex > 0) {
            var columns = NyckelDB.DB[this.id].columns, col = columns.$headers[colIndex], time = this.VALIDATE_EDIT_TIME.call(this, editTime, null, "column", null, "deleteColumn");
            for (var a = 0, len = NyckelDB.DB[this.id].table.length; a < len; a++) {
            ...
            if (storeBool !== false)
                this.TO_LOCAL_STORAGE.call(this, true);
            if (callback instanceof Function)
                return callback(true, NyckelDB.ERRORS[this.id], NyckelDB.DB[this.id].title, this.syncPending);
            else
                return true;
        }
        else
            return callback instanceof Function ? callback(false, colName + " column not found", NyckelDB.DB[this.id].title, this.syncPending) : false;
    };
    NyckelDB.prototype.ADD_COLUMN = function (colName, position, options, storeBool, editTime, callback) {
        ...
        function applyProps(cb) {
            for (var prop in options) {
                ...
                switch (prop) {
                    ...
                    case "initialValue":
                        if (this.VALUE_IS_VALID.call(this, options[prop][0], props.type[0]))
                            props[prop] = options[prop];
                        break;
                    case "search":
                        if (this.VALUE_IS_VALID.call(this, options[prop][0], "boolean"))
                            props[prop] = options[prop];
                        break;
        ...
        var orig = String(colName), i = 1, props = {
            type: ["any", 0],
            search: true
        }, cols = NyckelDB.DB[this.id].columns, headers = cols.$headers;
        colName = VAL.toPropName(colName);
        if (colName === "id")
            colName = "_id";
    ...
    NyckelDB.prototype.GET_COL_PROP = function (colName, propName) {
        var colIndex = this.GET_INDEX_OF_COLUMN.call(this, colName);
        if (colIndex > 0) {
    ...
    NyckelDB.prototype.SET_TYPE = function (colName, type, data, storeBool, editTime, callback) {
        function retError(err) {
            if (data)
                this.CACHE_ERROR.call(this, err);
        ...
        function setType() {
            if (can_do)
                NyckelDB.DB[this.id].columns[colName].type = [this.VALIDATE_TYPE.call(this, type), editTime];
        }
    ...
    NyckelDB.prototype.MOVE_COLUMN = function (colName, position, storeBool) {
        //TODO
    };
    NyckelDB.MAX_SYNC_FREQUENCY = 5; //5 minutes
    NyckelDB.SYNC_ERROR = false;
    NyckelDB.VALID_TYPES = new RegExp("^(" + ["any", "number", "integer", "posInteger", "negInteger", "boolean", "string",
    ...
    NyckelDB.VALID_STRING_TYPES = new RegExp("^(" + ["any", "string", "uniqueString", "multilineString", "date",
    ...
    NyckelDB.VALID_NUMBER_TYPES = new RegExp("^(" + ["any", "number", "integer", "posInteger", "negInteger", "date",
