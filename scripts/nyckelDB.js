...
APP.nyckelDB = (function () {
	...
	*/
		...
		function applyJSON(json) {
			...
			function syncTable(table, ids) {
				...
				function addNewRow(nRow) {
					if ((!DB[this.id].ids[nRow[0]] || DB[this.id].ids[nRow[0]][0] === "del" && DB[this.id].ids[nRow[0]][1] < ids[nRow[0]][0]) - createdDiff) {
						//new row
						syncChanges = true;
	...
	function SET_VAL(rowId, colName, newValue, storeBool, editTime, callback) {
		...
		else return callback instanceof Function ? callback(false, ERRORS[this.id], DB[this.id].title, this.syncPending) : false;
	}
	...
	/**
	 * forEachCol funct, use parameters 'is' and 'of' to get progress
	 * @callback eachColCallback
	 * @param {string} colName the name of the current column
	 * @param {number} index the position in the table of the current column
	 * @param {number} numCols the total number of columns in the table (% complete = index/numCols)
	 */
